...
public class QuizInterface extends javax.swing.JFrame {
    ...
    private String roll;
    private int[] result;
    private ArrayList<Good> easyQ;
    private ArrayList<Tough> medQ;
    private ArrayList<Complex> hardQ;
    ...
    private int good, epos, eneg;
    private int complex, mpos, mneg;
    private int tough, hpos, hneg;
     
    private ArrayList<Integer> arr = new ArrayList<>();
     
    void setArr(ArrayList<Integer> arr){
        try{  
            this.arr = arr;
            epos = arr.get(0);
            eneg = arr.get(1);
    ...
    void setEpos(int epos){
        this.epos = epos;
    }
    
    void setEneg(int eneg){
        this.eneg = eneg;
    }
    ...
    void setMneg(int mneg){
        this.mneg = mneg;
    }
    void setHpos(int mpos){
        this.mpos = mpos;
    }
    
    void setHneg(int hneg){
        this.hneg = hneg;
    }
    public QuizInterface(String _name, String _roll) {
        
        initComponents();
        
        this.easyQ = new ArrayList<>();
        this.medQ = new ArrayList<>();
        this.hardQ = new ArrayList<>();
    ...
    public void initializeInterface(){
        ...
        headerRollNumberLabel.setText(this.roll);
    ...
    public void fillQuestionsFromCSV(){
        ...
        // read GOOD Questions
        URL urlE = getClass().getResource("easyQ.csv");

        File questionFile = new File(urlE.getPath());
        ...
        try{
            ...
            while (inputStream.hasNextLine()){
                ...
                easyQ.add(new Good(values[0],options,Integer.parseInt(values[5])));    
            }
        ...
        URL urlM = getClass().getResource("medQ.csv");

        File questionFile1 = new File(urlM.getPath());
        ...
        try{
            ...
            while (inputStream.hasNextLine()){
                String ques = inputStream.nextLine();
                String[] values = ques.split(",");           // reading CSV file
                medQ.add(new Tough(values[0],Integer.parseInt(values[1])));    
            }
        }catch(FileNotFoundException e){
        }
        ...
        URL urlH = getClass().getResource("hardQ.csv");

        File questionFile2 = new File(urlE.getPath());
        ...
        try{
            ...
            while (inputStream.hasNextLine()){
                ...
                hardQ.add(new Complex(values[0],options,correctAns));    
            }
        }catch(FileNotFoundException e){
        }
    ...
    public void fetchNewQuestion(int _selectedType){
        ...
        if(_selectedType == 1){
            ...
            int index = random.nextInt(easyQ.size());
            
            //JOptionPane.showMessageDialog(null, easyQ.size());
            
            //fetch Good Question Here  
            
            activeQuestionList.add(easyQ.get(index));
            easyQ.remove(index);
            ...
            int index = random.nextInt(medQ.size());
            activeQuestionList.add(medQ.get(index));
            medQ.remove(index);
            
        } else if(_selectedType == 3){
            
            //generateRandomNumberAccordingToComplexQuestionSize
            int index = random.nextInt(hardQ.size());
            activeQuestionList.add(hardQ.get(index));
            hardQ.remove(index);
    ...
    private void initComponents() {
        ...
        headerRollNumberLabel.setText("IIT2018xyz");
